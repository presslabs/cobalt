...
from models.manager import VolumeSchema, VolumeAttributeSchema, VolumeManager
...
class TestVolumeIntegration:
    ...
    def test_volume_list_post_empty_body_no_json(self, volume_raw_empty,
                                                 flask_app):
        with flask_app.test_client() as c:
    ...
    def test_volume_list_post_empty_body(self, flask_app):
        expected_result = {'message': {
            'requested': {'constraints': ['Missing data for required field.'],
                          'reserved_size': [
                              'Missing data for required field.']}}}
        expected_code = 400

        with flask_app.test_client() as c:
            response = c.post('/volumes', data='{}',
    ...
    def test_volume_list_post_minimal_body(self, volume_raw_minimal,
                                           flask_app):
        expected_result = {
            'meta': {},
            'name': '',
        ...
        with flask_app.test_client() as c:
            response = c.post('/volumes', data=volume_raw_minimal,
                              content_type='application/json')
    ...
    @mark.parametrize('parent_state', ['deleting', 'scheduling'])
    ...
    def test_volume_list_post_read_only_and_extra_body(self,
                                                       volume_raw_read_only_extra,
                                                       flask_app):
        expected_result = {
        ...
        with flask_app.test_client() as c:
            response = c.post('/volumes', data=volume_raw_read_only_extra,
                              content_type='application/json')
    ...
    @mark.parametrize('method', ['put', 'get', 'delete'])
    ...
    @mark.parametrize('method,expected_message', [
    ...
    def test_volume_put_not_modified(self, etcd_client, volume_raw_ok_ready,
        ...
        with flask_app.test_client() as c:
            response = c.put('/volumes/{}'.format(id), data=requested,
                             content_type='application/json')
    ...
    def test_volume_put_extra_body(self, etcd_client, volume_raw_ok_ready,
        ...
        expected['state'] = 'pending'
        expected['requested'], errors = VolumeAttributeSchema().loads(
            volume_raw_requested_extra)
        assert errors == {}

        with flask_app.test_client() as c:
            response = c.put('/volumes/{}'.format(id),
                             data=volume_raw_requested_extra,
                             content_type='application/json')
